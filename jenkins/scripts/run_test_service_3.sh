...
bash -c "pylint-fail-under --fail_under 1 -f parseable src > src/tests/test_results/pylint_report.log"
...
bash -c "pycodestyle src > src/tests/test_results/pep8_report.txt || exit 0"